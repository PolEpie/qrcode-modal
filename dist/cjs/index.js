...
function formatDeepLink(entry) {
  return entry.deepLink ? ("" + (entry.deepLink) + (entry.deepLink.endsWith(":") ? "//" : "/") + "wc?uri=") : "";
}
...
function open$1(uri, cb, onClick, qrcodeModalOptions) {
  injectStyleSheet();
...
function open$2(uri, cb, onClick, qrcodeModalOptions) {
  console.log(uri);
  if (isNode()) {
    open(uri);
  } else {
    open$1(uri, cb, onClick, qrcodeModalOptions);
  }
}
function close$2() {
  if (isNode()) ; else {
